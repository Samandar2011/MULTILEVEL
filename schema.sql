...
-- Topshirilgan testlar: BITTA yakunlangan urinish = BITTA topshiriq.
CREATE TABLE submissions (
  id UUID PRIMARY KEY,
  attempt_id UUID NOT NULL UNIQUE REFERENCES test_attempts(id),          -- bitta urinish -> ko'pi bilan bitta topshiriq
  user_id UUID NOT NULL REFERENCES users(id),
  test_id UUID NOT NULL REFERENCES tests(id),
  category TEXT NOT NULL CHECK (category IN ('multilevel','speaking','listening','writing','reading')),
  status TEXT NOT NULL DEFAULT 'pending_admin' CHECK (status IN ('pending_admin','in_review','ready')),
  score NUMERIC,
...
-- Foydalanuvchining shu testdagi tekshirilmagan topshirig'i bittadan ortiq bo'lolmaydi
CREATE UNIQUE INDEX one_open_submission_per_user_test ON submissions (user_id, test_id) WHERE status <> 'ready';
CREATE INDEX submissions_admin_queue ON submissions (status, submitted_at DESC);
